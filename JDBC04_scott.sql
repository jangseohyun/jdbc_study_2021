...
--4. 이름 검색 쿼리문 구성
SELECT * FROM (SELECT SID,NAME,KOR,ENG,MAT,KOR+ENG+MAT AS TOT,(KOR+ENG+MAT)/3.0 AS AVG, RANK() OVER(ORDER BY (KOR+ENG+MAT) DESC) AS RANK FROM TBL_SCORE) WHERE NAME = '홍길동' ORDER BY SID ASC
;
